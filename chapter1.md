...
五个功能是：错误控制，流控制，片段化和重组，多任务和连接设置。

23

应用层：最靠近用户的层，处理特定的应用程序细节，主要为应用提供网络接口服务。

传输层：负责在网络中的两个点之间提供可靠的数据传输服务。

网络层：负责在多个网络间传输数据包，主要包括路由选择和数据包转发。

数据链路层：负责在相同的物理网络中的设备间进行流量传输。

物理层：负责在物理媒介上进行原始比特流传输。